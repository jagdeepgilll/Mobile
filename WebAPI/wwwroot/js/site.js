...
const uri = "/api/Mobiles"; //the api as a global variable
// alert("API " + uri);
let allMobiles = null; //holds the data in a global
...
function LoadTable() {
    $.ajax({
        ...
        success: function (data) { //if the request succeeds ....
            const tBody = $("#allMobiles"); //for the tbody bind with allstaff <tbody id = "allMobiles" ></tbody >
            allMobiles = data; //pass in all the data to the global allstaff use it in Edit
            ...
            $.each(data,
                function (key, item) {
                    const tr = $("<tr></tr>")
                        ...
                        .append($("<td></td>")
                            .append($("<a href='#editMobileModal' class='btn btn-warning' data-toggle='modal'>Edit</a>")
                                .on("click",
                                    function () {
                                        editItem(item.id);
                            ...
                            .append(
                                    $("<a href='#deleteItem' class='btn btn-danger'><span>Delete</span></a>").on("click",
                                        function () {
                                            deleteItem(item.id);
...
function getCount(data) {
    ...
    if (data) { //if any data exists
        ...
        theCount.text(`There are ${data} Mobiles`);
    } else {
        theCount.text("There are no Mobiles");
        alert("No data");
    }
}


//Delete a vehcle from the database
...
//click event for edit button to load details into form. Go through each entry held in  allMobiles and add in the one that matches the id from the click
